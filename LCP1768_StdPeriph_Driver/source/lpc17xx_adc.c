...
void ADC_Init(LPC_ADC_TypeDef *ADCx, uint32_t rate)
{
	...
	CLKPWR_ConfigPPWR (CLKPWR_PCONP_PCAD, ENABLE);		// Turn on power and clock
	ADCx->ADCR = 0;
	tmp = ADC_CR_PDN;									// Enable PDN bit
	ADCPClk = CLKPWR_GetPCLK(CLKPWR_PCLKSEL_ADC);		// Set clock frequency
...
{
	ADCx->ADCR &= ~ADC_CR_PDN;						// Clear PDN bit
	CLKPWR_ConfigPPWR (CLKPWR_PCONP_PCAD, DISABLE);	// Turn off power and clock
}
